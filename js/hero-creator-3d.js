...
(function () {
  ...
  function setupScene() {
    ...
    const ambientLight = new THREE.AmbientLight(0xf7f3ec, 1.15);
    ...
    const blueRim1 = new THREE.PointLight(0xa56f7d, 3.6, 12);
    blueRim1.position.set(-3.5, 2.5, 2);
    scene.add(blueRim1);

    // 림 라이트 2: 투명하고 차가운 일렉트릭 시안 / 아이스 아쿠아 (#06b6d4)
    const blueRim2 = new THREE.PointLight(0xb68894, 3.0, 12);
    blueRim2.position.set(3, -3, 2.5);
    scene.add(blueRim2);

    // 내부 발광 코어 라이트 (아이스 블루)
    const corePointLight = new THREE.PointLight(0xd6b7be, 1.3, 6);
    ...
    const ring1 = createKineticRing(1.95, 0.015, 0xbc8d9a, Math.PI / 3, 0);       // 아이스 블루
    const ring2 = createKineticRing(2.25, 0.012, 0xa56f7d, -Math.PI / 4, Math.PI / 6); // 코발트 일렉트릭 블루
    const ring3 = createKineticRing(2.5, 0.01, 0x8f5f6d, Math.PI / 6, -Math.PI / 3);  // 쿨 시안
    ringGroup.add(ring1);
    ringGroup.add(ring2);
    ringGroup.add(ring3);
    ...
    function createParticleTexture() {
      ...
      grad.addColorStop(0.35, 'rgba(165, 111, 125, 0.9)');
      grad.addColorStop(0.7, 'rgba(143, 95, 109, 0.3)');
